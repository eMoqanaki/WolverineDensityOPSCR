...
## -----------------------------------------------------------------------------
## ------ II. MODEL SETTING AND RUNNING ------- without RJMCMC
## -----------------------------------------------------------------------------
...
modelCode <- nimbleCode({
  ...
  for (t in 1:n.years) {
    for (h in 1:n.habCovs) { 
      betaHabCovs[h, t] <- beta0[h] + (beta1[h] * Time[t])
    }#h
    
    # 10 spatial covariates of density
...
nimParams <- c("N","beta0","beta1","betaHabCovs", 
               ...
               # , "habIntensity" #--useful for constructing density surfaces
               )
nimParams2 <- c("z","sxy")
...
## -----------------------------------------------------------------------------
## ------ III. MODEL FITTING ------- WITHOUT RJMCMC
## -----------------------------------------------------------------------------
...
# traceplot for abundance in year 1 as an example
plot(nimbleMCMC_samples[,'N[1]'], type = 'l', las = 1)
